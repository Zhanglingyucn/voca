...
{
private:
    std::vector<double> L1_min, L1_max;              // decompressed aggregate coefficients
    std::vector<double> L2_min, L2_max;              // decompressed detail coefficients
#ifdef COMPRESS_ALP
    std::vector<CompressedData *> L3_min, L3_max; // compressed detail coefficients
#else
    std::vector<std::vector<double>> L3_min, L3_max; // decompressed detail coefficients
#endif
    OrderArr bottom_order;
    size_t variable_id;
    size_t data_size;

    void build_multilevel_cache(const CoeffPair& top_aggregate, const CoeffArr &input_data, bool is_min);
    CoeffPair minmax_decode_pair(double agg_parent, double detail, bool is_min);
...
public:
    ...
    std::pair<CoeffPair, CoeffPair> minmax_decode_index(double agg_min, double agg_max, size_t index, size_t original_level);
...
public:
    ...
    std::pair<CoeffPair, CoeffPair> minmax_decode_index(double agg_min, double agg_max, size_t index, size_t variable_id);
    bool load_minmax_data();
